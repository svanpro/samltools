...
const metaDataObj = parser.parse(samlMetaData, options);

// Audience URI
const audienceUrl = metaDataObj.EntityDescriptor.attr_entityID;
console.log("Audience URI", audienceUrl)


// SAML REDIRECT URL and POST URL
const samlBindings = metaDataObj.EntityDescriptor.IDPSSODescriptor.SingleSignOnService;
const post_url = samlBindings.filter(binding => binding.attr_Binding == BINDING_HTTP_POST).map(binding => binding.attr_Binding);
const redirect_url = samlBindings.filter(binding => binding.attr_Binding == BINDING_HTTP_REDIRECT).map(binding => binding.attr_Binding);
